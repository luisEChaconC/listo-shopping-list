...
export default function PageName() {
  ...
  const product_map = Object.fromEntries(
    user_products.map(p => [p.id, p])
  );

  const associated_ids = new Set(
    list_asociate_products.map(p => p.product_id)
  );

  //Esta es para poder agregar los productos que no estan
  const not_list = user_products.filter(
    p => !associated_ids.has(p.id)
  );

  const [notList, setNotList] = useState<Product[]>(() =>
    user_products.filter(p => !associated_ids.has(p.id))
  );

  function add_product(prod: Product) {
    setNotList(prev => prev.filter(p => p.id !== prod.id));
  }
      ...
      <div className="w-full h-full bg-green-00 rounded-lg flex justify-center">
        <table className="table-fixed w-11/12 text-center align-middle">
          <thead>
            ...
            <tbody>
              {list_asociate_products.map((item, index) => (
                <tr key={index} className={
                  index === list_asociate_products.length - 1
                  ? "h-14"
                  : "h-14 border-b-[1px]"
                }>
